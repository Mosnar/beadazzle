...
- Large projects now expand and collapse outline rows without repeating the active filter or sort, change sort order without rerunning filters, and share normalized issue data across bookmark counts. Outline context uses less temporary memory, and only actual blocking relationships can override the requested sibling sort. This keeps disclosure clicks, sorting, and saved-view updates responsive on trackers with thousands of beads.
- You can now pin Beads state properties (the `dimension:value` labels written by `bd set-state`, such as `phase:implementation`) to every bead's Properties section from the new Project Settings › Properties pane. Properties already recorded in the project appear in a separate Other Properties section where they can be pinned from the context menu or dragged into the pinned order, while Add Property creates one before its first use. Customize pinned or unpinned project-local display names without renaming the underlying Beads identifiers or history, and review any selected property's recorded values in an independently scrolling pane beside the list. Each pinned row shows the bead's current value and opens a picker of the project's values for that dimension, with room for a custom value and an optional reason. Changes appear immediately even in large projects and run `bd set-state`, so they record a state-change event in Activity instead of silently rewriting labels.
- State values can now have project-local display names that can be reset at any time, show how many beads currently use them, and open an exact filtered bead list. Archive retired values to remove them from new selections without changing existing beads or their Activity history, then restore them whenever they are needed again.
- The label editor and full-bead saves now preserve pinned state properties across ordinary and concurrent label edits, so a state can't be accidentally removed or rolled back without a matching `bd set-state` event. State names and values remain case-sensitive, values containing commas or equals signs round-trip correctly, and properties containing Beads' event-title arrow remain discoverable. Unpinned state labels continue to behave as plain labels once any in-flight state change has reconciled.
