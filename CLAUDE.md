...
**App wiring (`Sources/Beadazzle/App/BeadazzleApp.swift`).** Menu commands communicate with views via `NotificationCenter` (`.newBeadRequested`, `.openProjectRequested`, `.refreshRequested`, `.focusSearchRequested`, etc.) — see `Support/Notifications.swift`. The app has three scenes: main window, Settings, and a `URL`-parameterized Project Settings window.
