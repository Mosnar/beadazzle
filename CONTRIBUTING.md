...
- Install `bd` if you want to exercise write flows locally, and point the app at a current Dolt-backed Beads project (embedded, server, or shared-server mode). Legacy SQLite-backed projects are unsupported.
...
- If you touch app behavior, run the relevant tests plus `./script/build_and_run.sh --verify` before opening a PR.
- Record user-facing changes in `CHANGELOG.md` under `## [Unreleased]`, written for users of the app rather than as a commit summary. Skip internal-only churn a user would never notice.
